...
class SystemMonitor(QWidget):
    ...
    def check_mouse_position(self):
        ...
        if (cursor_pos.y() <= self.y() + self.height() + 30) and (abs(cursor_pos.x() - screen_rect.width() // 2) <= self.width() // 2 + 30):
...
class Systeminfo_Thread(QThread):
    ...
    def get_system_info(self, sensors):
        ...
        try:
            ...
            return (f'CPU : {color_print(cpu_usage, 1)} % | CPU Clock : {cpu_clock} MHz | CPU TEMP : {color_print(cpu_temp, 2)}°C<br>'
                    f'GPU : {color_print(gpu_usage, 1)}% | GPU Clock : {gpu_clock} MHz | GPU TEMP : {color_print(gpu_temp, 2)}°C<br><br>'
                    ...
                    f'POWER : {color_print(total_power, 4)} W | Net : {color_print(net_sent_speed, 5)}MB/s ↑ {color_print(net_recv_speed, 5)}MB/s ↓')
